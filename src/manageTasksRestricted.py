    ...
    while len(runList) > 0:
        ...
        if len(jobList) < numWorkers:
            ...
            if nProcs > 0:
                ...
                fid = open(fileName, 'w')
                fid.write('mpiexec -npernode 2' + ' -wdir /shared/users/dstrauss/dev/subsurface/src python coordinate.py ' + sys.argv[1] + ' ' + repr(ix))
                fid.close()
                cmd = ['qsub', '-N', jobTitle, '-l' , 'walltime=20:00:00', '-l','nodes=' + repr(nProcs) + ':ppn=4', '-l', 'nice=0', '-q','batch', fileName]        
                print cmd
