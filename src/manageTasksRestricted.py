...
@author: dstrauss
 Main difference between this and the other manageTasks is that I'm going to try to get
 only a small number of independent conditions per node - and restrict it that way
    ...
    while len(runList) > 0:
        ...
        if len(jobList) < numWorkers:
            ...
            if nProcs > 0:
                ...
                fid = open(fileName, 'w')
                fid.write('mpiexec' + ' -wdir /shared/users/dstrauss/dev/subsurface/src python coordinate.py ' + sys.argv[1] + ' ' + repr(ix))
                fid.close()
                cmd = ['qsub', '-N', jobTitle, '-l' , 'walltime=20:00:00', '-l','nodes=' + repr(nProcs) + ':ppn=4', '-l', 'nice=0','-l','procs_bitmap=1000', fileName]        
                print cmd
